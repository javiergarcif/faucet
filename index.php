...
if (isset($_SESSION['address_input_name'.$session_prefix]) && array_key_exists($_SESSION['address_input_name'.$session_prefix], $_POST)) {
    $_POST["address"] = trim($_POST[$_SESSION['address_input_name'.$session_prefix]]);
} else {
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        if (isset($_SESSION['address_input_name'.$session_prefix])) {
...
if (array_key_exists('address', $_POST)) {
    ...
    $data["reflink"] .= trim($_COOKIE['address_'.$data['currency']]);
    $data["address"] = trim($_COOKIE['address_'.$data['currency']]);
} else if (array_key_exists('address', $_COOKIE)) {
    $data["reflink"] .= trim($_COOKIE['address']);
    $data["address"] = trim($_COOKIE['address']);
} else {
    ...
    if ($error) {
        ...
        if ($faucet_settings_array['service']=='expresscrypto') {
            if ((strpos($ret['html'], 'This address is not linked to any account in ExpressCrypto.io')!==false) || (stripos($address, 'EC-')===false)) {
                ...
                $ret['html'] = '<div class="alert alert-danger">This address is not linked to any account in ExpressCrypto.io. <a href="http://expresscrypto.io/" onmousedown="$(this).attr(\'href\', \''.$expresscrypto_ref_url.'\');" target="_blank">Make an account</a>.</div>';
            }
        }
        if (strpos($ret['html'], 'make an account')!==false) {
            if (empty($faucethub_ref_url)) {
                $faucethub_ref_url = 'http://faucethub.io/r/13852';
            }
