...
if (isset($_SESSION['logged_in'.$session_prefix])) { // logged in to admin page
    ...
    $currencies = array('BTC', 'BCH', 'BCN', 'BLK', 'DASH', 'DGB', 'DOGE', 'ETH', 'EXG', 'EXS', 'HORA', 'LSK', 'LTC', 'NEO', 'POT', 'PPC', 'STRAT', 'TRX', 'WAVES', 'XMR', 'XPM', 'XRP', 'ZEC');
    $send_coins_message = '';
    ...
    if ($row = $q->fetch()) {
        ...
        if ($update_last_check<time()-120) {
            ...
            if ($ch = curl_init()) {
                ...
                curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0');
        ...
        if ((!empty($update_data['shortlink_providers']))&&(is_array($update_data['shortlink_providers']))) {
            ...
            foreach ($update_data['shortlink_providers'] as $shortlink_key => $shortlink_value) {
                ...
                if (!empty($shortlink_value['count_shortlink'])) {
                    ...
                    $html_performance.= '<span title="Direct rewards">&#x1F4B4;</span> '.$shortlink_value['sum_reward'];
                    if ($currency == 'DOGE') {
                ...
                /*
                // 24h
                if (isset($shortlink_value['sum_users_sent_24h'])) {
                    $html_performance.= '<span title="Users sent">&#x1F517;</span> '.$shortlink_value['sum_users_sent_24h'].' &nbsp; ';
                    $html_performance.= '<span title="Users returned">&#x1F44D;</span> '.$shortlink_value['sum_users_returned_24h'].' &nbsp; ';
                    // show percentage
                    if ($shortlink_value['sum_users_sent_24h']>0) {
                        $html_performance.= '<span title="Rate">&#x2696;</span> '.round($shortlink_value['sum_users_returned_24h']*100/$shortlink_value['sum_users_sent_24h'], 1).'% &nbsp; ';
                    } else {
                        $html_performance.= '<span title="Rate">&#x2696;</span> N/A &nbsp; ';
                    }
                }
                */
                // 7d
                if (isset($shortlink_value['sum_users_sent_7d'])) {
                    $html_performance.= ' &nbsp; <span title="7 days stats">&#x1F5D3;</span> ';
                    ...
                    if ($shortlink_value['sum_users_sent_7d']>0) {
                        $html_performance.= '<span title="Rate">&#x2696;</span> '.round($shortlink_value['sum_users_returned_7d']*100/$shortlink_value['sum_users_sent_7d'], 1).'%';
                    } else {
                        $html_performance.= '<span title="Rate">&#x2696;</span> N/A';
                    }
                }
